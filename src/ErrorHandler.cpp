...
void ErrorHandler::LogWin32Error(const std::string &operation)
{
    ...
    if (size > 0 && messageBuffer)
    {
        ...
        std::cerr << "[-] " << operation << " failed. Error " << error << ": " << messageBuffer << std::endl;
        LocalFree(messageBuffer);
    }
    else
    {
        std::cerr << "[-] " << operation << " failed. Error: 0x" << std::hex << error << std::dec << std::endl;
    }
}
...
void ErrorHandler::LogNTStatus(const std::string &operation, NTSTATUS status)
{
    ...
    if (hNtdll)
    {
        size = FormatMessageA(
            ...
            hNtdll, static_cast<DWORD>(status), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
            reinterpret_cast<LPSTR>(&messageBuffer), 0, NULL);
    }
